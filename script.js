...
function bookingToRow(b){
  return {
    ...
    freq: b.freq, count: b.count, notes: b.notes,
...
function rowToBooking(r){
  return {
    ...
    freq: r.freq, count: r.count, notes: r.notes,
...
async function pushBookingsToCloud(arr){
  ...
  const rows = arr.map(bookingToRow);
  const { error } = await sb.from('bookings').upsert(rows);
  if(error){ console.error('Supabase bookings upsert failed:', error); showToast('Could not save booking to the database — check your connection.','error'); }
}
...
async function pullBookingsFromCloud(){
  const { data, error } = await sb.from('bookings').select('*').order('submitted', { ascending: false });
...
function selectExportType(type, el){
  exportType = type;
  document.querySelectorAll('#etype-report,#etype-raw').forEach(e=>e.classList.remove('sel'));
  el.classList.add('sel');
  const filters = document.getElementById('report-filters');
  if(filters) filters.style.display = type==='raw' ? 'none' : '';
}
function openExportModal(){
  ...
  selectExportType('report', document.getElementById('etype-report'));
...
function runExport(){
  if(exportType==='raw'){ exportDatabaseTables(); return; }
  if(exportFmt==='csv') exportToCSV(); else exportToExcel();
}

/* ── Raw database export: uses the same data already in memory, but
   shaped exactly like the database rows (DB column names, e.g.
   start_time/end_time/account_id instead of start/end/accountId) —
   no network call, just a different column format than the report. ── */
function exportDatabaseTables(){
  bookings = loadBookings();
  zoomAccounts = loadAccounts();
  const bookingRows = bookings.map(bookingToRow);
  const accountRows = zoomAccounts; // account fields already match DB columns 1:1
  const stamp = toLocalISODate(new Date());

  if(exportFmt === 'csv'){
    downloadCSV(bookingRows, `bookings-table-${stamp}.csv`);
    downloadCSV(accountRows, `accounts-table-${stamp}.csv`);
  } else {
    if(typeof XLSX === 'undefined'){ showToast('Excel export library failed to load.','error'); return; }
    const wb = XLSX.utils.book_new();
    ...
    XLSX.writeFile(wb, `database-export-${stamp}.xlsx`);
  }
  showToast(`Exported ${bookingRows.length} booking row(s) and ${accountRows.length} account row(s) in database format.`,'success');
  closeOverlay('export-ov');
}
/* Generic array-of-objects -> CSV downloader, used for raw table export
   (column set isn't fixed ahead of time like the filtered report is). */
