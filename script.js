...
function recurLabel(b){
  ...
  const daysPart = (b.freq==='weekly' && b.days && b.days.length) ? ` (${b.days.join(', ')})` : '';
...
function sendEmail(b){
  try{
    ...
    emailjs.send(EMAILJS_SVC,EMAILJS_TPL,{
      ...
      recurring:b.recurring?'Yes ('+recurLabel(b)+')':'No',
      notes:b.notes||'—',
    });
...
function onFreqChange(){
  ...
  wrap.style.display = freq==='weekly' ? 'block' : 'none';
  const hint = document.getElementById('count-hint');
  hint.textContent = freq==='weekly' ? 'Number of weeks to repeat' : 'Total number of occurrences';
  if(freq==='weekly') onDaysChange();
}
...
function submitBooking(){
  ...
  const days  = (recur && freq==='weekly') ? getSelectedDays() : [];

  if(!name||!agency||!title||!date){showToast('Please fill in all required fields.','error');return;}
  if(recur && freq==='weekly' && !days.length){showToast('Select at least one day to repeat on.','error');return;}
...
function finalizeBookingSubmit(data){
  ...
  const bk = {id:newId,title,name,agency,date,shift:selShiftV,start,end,pax,accountId,
    recurring:recur,freq:recur?freq:'',count:recur?count:0,days:(recur&&freq==='weekly')?days:[],notes,
