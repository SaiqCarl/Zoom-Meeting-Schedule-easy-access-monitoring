...
function rebuildUserRoomButtons(){
  ...
  // Also update export pax select — list every room (active or expired) by name,
  // so admins can tell rooms apart instead of just seeing a capacity number.
  // Deleted rooms are never in zoomAccounts, so they're naturally excluded.
  const expPax = document.getElementById('exp-pax');
  if(expPax){
    const prevExpPaxVal = expPax.value;
    const allRoomsSorted = zoomAccounts.slice().sort((a,b)=>{
      const na = parseInt(a.capacity), nb = parseInt(b.capacity);
      if(na !== nb) return na - nb;
      return a.name.localeCompare(b.name);
    });
    let opts = '<option value="ALL">All Rooms</option>';
    allRoomsSorted.forEach(acc => {
      const expiredNote = acc.status === 'expired' ? ' — Expired' : '';
      opts += `<option value="${acc.id}">${acc.name} (${acc.capacity} pax)${expiredNote}</option>`;
    });
    expPax.innerHTML = opts;
    if(prevExpPaxVal && Array.from(expPax.options).some(o=>o.value===prevExpPaxVal)) expPax.value = prevExpPaxVal;
  }
...
/* Build the display name + status for a booking's room, even if the account
   is expired (still shown, with a note) or has since been deleted (booking is
   still shown, but the room is labeled as removed). */
function roomInfoForBooking(b, accountsList){
  const acc = accountsList.find(a=>a.id===b.accountId);
  if(!acc){
    return { name: b.accountId ? 'Deleted Room' : '—', status: b.accountId ? 'Deleted' : '—' };
  }
  return { name: acc.name, status: acc.status==='expired' ? 'Expired' : 'Active' };
}
function exportToExcel(){
  ...
  const room  = document.getElementById('exp-pax').value; // 'ALL' or a room/account id
  ...
  bookings = loadBookings();
  const allAccounts = loadAccounts(); // active + expired rooms; deleted rooms are already absent
  const filtered = sortByNewest(bookings).filter(b=>(month==='ALL'||b.date.slice(0,7)===month)&&(room==='ALL'||b.accountId===room)&&statuses.includes(b.status));
  if(!filtered.length){ showToast('No matching requests to export.','error'); return; }
  const rows = filtered.map(b=>{
    const info = roomInfoForBooking(b, allAccounts);
    return {'ID':b.id,'Title':b.title,'Requestor':b.name,'Agency':b.agency,'Date':b.date,'Shift':b.shift,'Start':b.start,'End':b.end,'Pax Capacity':b.pax,'Room Name':info.name,'Room Status':info.status,'Recurring':b.recurring?recurLabel(b):'No','Registration':b.registration?'Required':'Not required','Security':securityLabel(b.security),'Status':b.status,'Submitted':b.submitted,'Notes':b.notes||''};
  });
  const ws = XLSX.utils.json_to_sheet(rows);
  ws['!cols']=[{wch:9},{wch:26},{wch:16},{wch:18},{wch:11},{wch:7},{wch:7},{wch:7},{wch:9},{wch:20},{wch:10},{wch:14},{wch:14},{wch:24},{wch:10},{wch:11},{wch:24}];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Bookings');
  const roomLabel = room==='ALL' ? 'all-rooms' : (allAccounts.find(a=>a.id===room)?.name || 'room').replace(/[^a-z0-9]+/gi,'-').toLowerCase();
  const fname=`zoom-bookings-${month==='ALL'?'all-months':month}-${roomLabel}.xlsx`;
...
function exportToCSV(){
  ...
  const headers=['ID','Title','Requestor','Agency','Date','Shift','Start','End','Pax Capacity','Room Name','Room Status','Recurring','Registration','Security','Status','Submitted','Notes'];
  const escape=v=>{const s=String(v??'');return s.includes(',')||s.includes('"')||s.includes('\n')?`"${s.replace(/"/g,'""')}"`  :s;};
  const rows=[headers.join(',')];
  filtered.forEach(b=>{
    const info = roomInfoForBooking(b, allAccounts);
    rows.push([b.id,b.title,b.name,b.agency,b.date,b.shift,b.start,b.end,b.pax,info.name,info.status,b.recurring?recurLabel(b):'No',b.registration?'Required':'Not required',securityLabel(b.security),b.status,b.submitted,b.notes||''].map(escape).join(','));
  });
  const blob=new Blob([rows.join('\n')],{type:'text/csv;charset=utf-8;'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
  const roomLabel = room==='ALL' ? 'all-rooms' : (allAccounts.find(a=>a.id===room)?.name || 'room').replace(/[^a-z0-9]+/gi,'-').toLowerCase();
  const fname=`zoom-bookings-${month==='ALL'?'all-months':month}-${roomLabel}.csv`;
  a.href=url;a.download=fname;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
  showToast(`Exported ${filtered.length} request(s) to ${fname}`,'success');
  closeOverlay('export-ov');
}
