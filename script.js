...
function submitBooking(){
  ...
  const bookingData = {name,agency,title,date,start,end,pax,notes,recur,accountId,freq,count,registration,security};
...
function finalizeBookingSubmit(data){
  const {name,agency,title,date,start,end,pax,notes,recur,accountId,freq,count,registration,security} = data;
  bookings = loadBookings();
...
function exportToExcel(){
  ...
  const rows = filtered.map(b=>({'ID':b.id,'Title':b.title,'Requestor':b.name,'Agency':b.agency,'Date':b.date,'Shift':b.shift,'Start':b.start,'End':b.end,'Pax Room':b.pax,'Recurring':b.recurring?`${b.freq} × ${b.count}`:'No','Registration':b.registration?'Required':'Not required','Security':securityLabel(b.security),'Status':b.status,'Submitted':b.submitted,'Notes':b.notes||''}));
...
function exportToCSV(){
  ...
  filtered.forEach(b=>rows.push([b.id,b.title,b.name,b.agency,b.date,b.shift,b.start,b.end,b.pax,b.recurring?`${b.freq} x ${b.count}`:'No',b.registration?'Required':'Not required',securityLabel(b.security),b.status,b.submitted,b.notes||''].map(escape).join(',')));
